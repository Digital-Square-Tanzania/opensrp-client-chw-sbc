...
public class SbcUtil {
    ...
    public static void processEvent(AllSharedPreferences allSharedPreferences, Event baseEvent) throws Exception {
        if (baseEvent != null) {
            SbcJsonFormUtils.tagEvent(allSharedPreferences, baseEvent);
            JSONObject eventJson = new JSONObject(SbcJsonFormUtils.gson.toJson(baseEvent));
    ...
    public static boolean launchDialer(final Activity activity, final BaseSbcCallDialogContract.View callView, final String phoneNumber) {
        if (ContextCompat.checkSelfPermission(activity, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {

            // set a pending call execution request
            if (callView != null) {
                callView.setPendingCallRequest(() -> SbcUtil.launchDialer(activity, callView, phoneNumber));
            }
    ...
    public static void saveFormEvent(final String jsonString) throws Exception {
        ...
        Event baseEvent = SbcJsonFormUtils.processJsonForm(allSharedPreferences, jsonString);
        SbcUtil.processEvent(allSharedPreferences, baseEvent);
    }
    ...
    public static class CloseSbcMemberFromRegister extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            SbcDao.closeSbcMemberFromRegister(baseEntityId);
            return null;
        }
